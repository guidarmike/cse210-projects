...
class Program
{
        ...
        if (int.TryParse(Console.ReadLine(), out int points))
        {
            ...
            if (int.TryParse(Console.ReadLine(), out int targetCount))
            {
                ...
                if (int.TryParse(Console.ReadLine(), out int bonusPoints))
                {
                    ...
                    user.GetGoals().Add(checklistGoal);
    ...
    static void SaveGoalsToFile(User user, string filename)
    {
        using (System.IO.StreamWriter file = new System.IO.StreamWriter(filename))
        {
            ...
            foreach (Activity goal in user.GetGoals())
            {
                string goalLine;
                ...
                {
                    goalLine = $"ChecklistGoal: \"{goal.GetName()}\", \"{goal.GetDescription()}\", {goal.GetPoints()}, {((ChecklistGoal)goal).BonusPoints}, {((ChecklistGoal)goal).TargetCount}, {((ChecklistGoal)goal)._currentCount}";
                }
                else
                {
                    continue;
    ...
    {
        List<Activity> goals = user.GetGoals();
    ...
    static void ListGoals(User user)
    {
        ...
        {
            Activity goal = user.GetGoals()[i];
            string status = goal.Completed ? "[X]" : "[ ]";
            string description = goal is ChecklistGoal ? $" -- Currently completed: {((ChecklistGoal)goal)._currentCount}/{((ChecklistGoal)goal).TargetCount} times" : "";
